...
Define $\vec{v}$ as the vector of voltages, $\vec{i}_0(t)$ as the vector of currents coming from input, $\vec{i}_{\mathrm{out}}(t)$ as the currents coming from other neurons, $\vec{i}_{\mathrm{tot}}(t) = \vec{i}_0(t) + \vec{i}_{\mathrm{out}}(t)$ as the total currents, and $\vec{w}_i$ as the weights vector corresponding to how other neurons connect to neuron $i$.
