...
def runge_kutta_method(x_0 : np.ndarray, time_array : np.ndarray, dt : float, I, tuple_of_constants : tuple[float]) -> np.ndarray:
    '''
    The function calculates numerical values for voltage V in HHM for a single cell under some current I. The function uses the Euler methode for calculation.
    :param x_0: initial values for voltage V and gating variables n,m,h.
    :param time_array: time interval the function calculate state of the neuron
    :param dt: time step.
    ...
    for t in time_array:
        ...
        k1 = dt * eq.equations_vector(x_0=x, I = I, tuple_of_constants = tuple_of_constants)
        k2 = dt * eq.equations_vector(x_0=x + k1 / 2, I = I, tuple_of_constants = tuple_of_constants)
        k3 = dt * eq.equations_vector(x_0=x + k2 / 2, I = I, tuple_of_constants = tuple_of_constants)
        k4 = dt * eq.equations_vector(x_0=x + k3, I = I, tuple_of_constants = tuple_of_constants)
